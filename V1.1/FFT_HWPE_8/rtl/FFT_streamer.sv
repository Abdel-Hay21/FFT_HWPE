...
module FFT_streamer #(
  ...
  generate
    if(TCDM_FIFO_DEPTH > 0) begin : use_fifo_gen
      ...
      hci_core_load_store_mixer #(
        ...
        .rst_ni   ( rst_ni               ),
        .clear_i  ( clear_i              ),
        .in_load  ( virt_tcdm[0]         ), //slave
        .in_store ( virt_tcdm[1]         ), //slave
        .out      ( tcdm_prefifo         )  //master
      );
      ...
      hci_core_fifo #(
        ...
        .tcdm_slave  ( tcdm_prefifo                ), //slave
        .tcdm_master ( tcdm_prefilter[0]           )  //master
      );
    end
  ...
  hci_core_r_valid_filter i_tcdm_filter (
    ...
    .clear_i     ( clear_i              ),
    .enable_i    ( 1'b1                 ),
    .tcdm_slave  ( tcdm_prefilter[0]    ), //slave
    .tcdm_master ( tcdm                 )  //master
  );
  assign flags_o.tcdm_fifo_empty = tcdm_fifo_flags.empty;

endmodule // FFT_streamer
