...
(
  ...
  localparam int unsigned ENG_BW = 8 * 32; 
 hwpe_ctrl_intf_periph #(.ID_WIDTH(ID_WIDTH)) periph_FFT [7:0] (.clk(clk));
...
generate
  for(genvar i=0; i<8; i++) begin : FFT_gen
    FFT_top #(.ID(ID_WIDTH), .N_CORES(N_CORES), .BW(256)) i_fft (
      .clk_i(clk), .rst_ni(rst_n), .test_mode_i(test_mode),
      .evt_o(evt_internal[i]), .tcdm(s_hci_hwpe_FFT[i]), .periph(periph_FFT[i])
    );
...
endgenerate
    ...
    // Latch the selection bit
  logic [2:0] r_sel_reg;
  // 2. Decoder
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) r_sel_reg <= 3'b0;
    else if (hwpe_cfg_slave.req && hwpe_cfg_slave.gnt) r_sel_reg <= hwpe_cfg_slave.add[9:7];
  end



// 2. 8-Way Address Decoder & Signal Routing (Fully Unrolled)
always_comb begin
  // ENGINE 0
  ...
  // Address Decoder
  if (hwpe_cfg_slave.req && (hwpe_cfg_slave.add[11:10] == 2'b0)) begin
  ...
  // 3. Responses/Grant
  // 3. Grant Routing (Explicitly unrolled to avoid error)
  always_comb begin
    case (hwpe_cfg_slave.add[9:7])
  ...
  always_comb begin
    case (r_sel_reg)
      3'b000: begin hwpe_cfg_slave.r_valid = periph_FFT[0].r_valid; hwpe_cfg_slave.r_rdata = periph_FFT[0].r_data; hwpe_cfg_slave.r_id = periph_FFT[0].r_id; end
      3'b001: begin hwpe_cfg_slave.r_valid = periph_FFT[1].r_valid; hwpe_cfg_slave.r_rdata = periph_FFT[1].r_data; hwpe_cfg_slave.r_id = periph_FFT[1].r_id; end
      3'b010: begin hwpe_cfg_slave.r_valid = periph_FFT[2].r_valid; hwpe_cfg_slave.r_rdata = periph_FFT[2].r_data; hwpe_cfg_slave.r_id = periph_FFT[2].r_id; end
      3'b011: begin hwpe_cfg_slave.r_valid = periph_FFT[3].r_valid; hwpe_cfg_slave.r_rdata = periph_FFT[3].r_data; hwpe_cfg_slave.r_id = periph_FFT[3].r_id; end
      3'b100: begin hwpe_cfg_slave.r_valid = periph_FFT[4].r_valid; hwpe_cfg_slave.r_rdata = periph_FFT[4].r_data; hwpe_cfg_slave.r_id = periph_FFT[4].r_id; end
      3'b101: begin hwpe_cfg_slave.r_valid = periph_FFT[5].r_valid; hwpe_cfg_slave.r_rdata = periph_FFT[5].r_data; hwpe_cfg_slave.r_id = periph_FFT[5].r_id; end
      3'b110: begin hwpe_cfg_slave.r_valid = periph_FFT[6].r_valid; hwpe_cfg_slave.r_rdata = periph_FFT[6].r_data; hwpe_cfg_slave.r_id = periph_FFT[6].r_id; end
      3'b111: begin hwpe_cfg_slave.r_valid = periph_FFT[7].r_valid; hwpe_cfg_slave.r_rdata = periph_FFT[7].r_data; hwpe_cfg_slave.r_id = periph_FFT[7].r_id; end
    endcase
  end
