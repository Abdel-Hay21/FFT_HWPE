...
module hci_interconnect #(
  ...
  // Instantiate 8 Splitters with explicit individual port binding
  generate
    for(genvar i=0; i<8; i++) begin : hwpe_splitter_gen
      
      // 1. Create a local 8-port array for this specific splitter
      hci_core_intf #(
        .DW ( 32 ), .AW ( AWC ), .BW ( 4 ), .UW ( UW_LIC )
      ) i_local_tcdm_master [7:0] (.clk(clk_i));
      
      // 2. Explicitly bind the local ports to the main 64-port array 
      //    (This avoids the "Index range is not constant" error)
      for(genvar j=0; j<8; j++) begin : i_binding
        hci_core_assign i_bridge (
           .tcdm_slave  ( i_local_tcdm_master[j] ), 
           .tcdm_master ( hwpe_narrow[i*8+j]      )
        );
      end

      // 3. Instantiate the Splitter using the local array
      hci_core_split #(
        .DW           ( 256 ),
        .NB_OUT_CHAN  ( 8   )
      ) i_splitter (
        ...
        .clear_i     ( clear_i              ),
        .tcdm_slave  ( hwpe[i]              ),
        .tcdm_master ( i_local_tcdm_master  )
      );
